...
public class CustomeExceptionHandler {
    ...
    private static final Logger log = LoggerFactory.getLogger(CustomeExceptionHandler.class);
    ...
    public ResponseEntity<?> forbiddenException(CustomForbiddenException e) {
        log.error(e.getMessage());
        return new ResponseEntity<>(new ResponseDto<>(-1, e.getMessage(), null), HttpStatus.FORBIDDEN); // 403 권한이 없음.
    }
